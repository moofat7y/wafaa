...
export default function Courses() {
  const courses_list = courses.map((item, index) => {
    return index % 2 === 0 ? (
      <div
        key={index}
        className={`${
      ...
      >
        ...
        <div
          ...
          className="order-1 relative before:absolute before:content-[''] before:start-0 before:top-0 before:w-1 before:h-full before:bg-[#965c43] bg-white shadow-xl w-full z-10 md:w-5/12 px-6 py-4"
        >
  ...
  return (
    <section className="py-6">
      <MainHeader title={"Courses"} icon={<BiSolidBookBookmark />} />
